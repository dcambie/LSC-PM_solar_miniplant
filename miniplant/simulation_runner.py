...
def _common_simulation_runner(scene: Scene, num_photons: int = 100, render: bool = False, workers: int = None):
    ...
    if workers == 1:
        ...
        if render:
            logger.warning("Cannot use renderer in multi-threaded simulations! Set workers to 1 for single-theaded")
        # MULTI-THREADED
...
def run_direct_simulation(tilt_angle: int = 0, solar_elevation: int = 30, solar_azimuth: int = 180,
    ...
    scene = create_direct_scene(tilt_angle=tilt_angle, solar_elevation=solar_elevation, solar_azimuth=solar_azimuth,
                                solar_spectrum_function=solar_spectrum_function)
    return _common_simulation_runner(scene, num_photons, render, workers)


def run_diffuse_simulation(tilt_angle: int = 0, solar_spectrum_function: Callable = lambda: 555,
                           num_photons: int = 100, render: bool = False, workers: int = None) -> float:
    # Create scene with the provided parameters
